(function () {
  ...
  var config = Object.assign({ submissionEndpoint: (window.ELAINE_PORTAL_CONFIG || {}).submissionEndpoint || '' }, window.ELAINE_BOOSTER_CONFIG || {});
  function portalAccessToken() { return window.ElainePortalAccess ? window.ElainePortalAccess.getToken() : ''; }
  var portalReady = window.ElainePortalAccess && window.ElainePortalAccess.ready
    ? window.ElainePortalAccess.ready
    : Promise.resolve('');
  var assignmentId = config.assignmentId || 'ela-skill-booster';
  ...
  var storageKey = 'elaine-booster:' + assignmentId + (testMode ? ':teacher-test' : '');
  var questions = [];
  var state = null;
  var saveTimer = null;
  var flagTimer = null;
  ...
  function installHeader() {
    ...
    header.innerHTML =
      '<a class="portal-brand" href="' + escapeHtml(config.returnUrl || '../') + '">Elaine SAT</a>' +
  ...
  function renderProgressReview(data) {
    ...
    state.studentName = data.studentName || 'Elaine';
    ...
    resultBox.innerHTML = '<span class="result-kicker">' + (teacherSubmitted ? 'Submitted by Teacher' : 'Live student progress') + '</span><div class="result-score"><strong>' +
      ...
      '</strong><span>' + (teacherSubmitted ? 'answers preserved' : 'answers currently synced') + '</span></div><p>' + (teacherSubmitted ? 'The Teacher marked this saved work as submitted without creating a score or graded result.' : 'Read-only view of Elaine’s latest saved work. This page refreshes automatically every five seconds.') + ' Last server update: <strong>' +
  ...
  function renderSubmittedDifficultyFlags(flags) {
    ...
    panel.hidden = false;
    panel.innerHTML = '<div><span class="result-kicker">Elaine’s submitted review list</span><h2>' +
      (numbers.length ? 'Questions Elaine ticked for review' : 'No questions were ticked for review') +
  ...
  function jsonp(action, parameters, success, failure) {
    ...
    var callbackName = '__elaineBooster' + Date.now() + Math.random().toString(16).slice(2);
