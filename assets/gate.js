(() => {
  const sessionAccessKey = 'elaine-sat-access';
  const trustedBrowserKey = 'elaine-sat-trusted-browser-v1';
  const deviceTokenKey = 'elaine-sat-device-token-v1';
  const tokenKey = 'elaine-sat-access-token';
  const expiryKey = 'elaine-sat-access-expires-at';
  const expectedHash = 'd1257eb9c5c27a5f7b782c773c64b0b6322eab99b3ee22cde7a645b28d5260de';
  const portalConfig = window.ELAINE_PORTAL_CONFIG || {};
  let resolveReady;
  let renewal = null;
  const ready = new Promise((resolve) => { resolveReady = resolve; });

  window.ElainePortalAccess = {
    ready,
  ...
  const consumeTrustedDevice = () => {
    try {
      ...
      const token = String(parameters.get('elaine-trusted-device') || '').replace(/[^a-f0-9]/gi, '').slice(0, 128);
      if (!token) return '';
  ...
  const requestData = (action, parameters) => new Promise((resolve, reject) => {
    ...
    const callbackName = `__elaineAccess${Date.now()}${Math.random().toString(16).slice(2)}`;
  ...
  async function ensureFreshAccess(force) {
    ...
    const token = window.ElainePortalAccess.getToken();
  ...
  const showGate = () => {
    ...
    gate.innerHTML = `
      <form class="access-card">
        <p class="access-kicker">Elaine · SAT Reading & Writing</p>
