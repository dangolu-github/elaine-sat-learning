(function () {
  ...
  var ENDPOINT = String((window.ELAINE_PORTAL_CONFIG || {}).submissionEndpoint || '');
  var state = {
    token: '',
    page: 1,
    totalPages: 1,
  ...
  onReady(function () {
    ...
    if (!window.ElainePortalAccess || !window.ElainePortalAccess.ready) {
      showLogbookStatus('Portal access could not be initialized.', true);
      return;
    }
    window.ElainePortalAccess.ready.then(function () {
      state.token = window.ElainePortalAccess.getToken ? window.ElainePortalAccess.getToken() : '';
      if (!state.token) {
  ...
  function request(action, parameters, success, failure) {
    ...
    var callbackName = '__elaineMistakeLogbook' + Date.now() + Math.random().toString(16).slice(2);
