(function () {
  ...
  var config = window.ELAINE_ASSIGNMENT_CONFIG || {};
  var portalConfig = window.ELAINE_PORTAL_CONFIG || {};
  var imageVersion = '?v=20260926-elaine';
  ...
  var saveTimer = null;
  var portalReady = window.ElainePortalAccess && window.ElainePortalAccess.ready ? window.ElainePortalAccess.ready : Promise.resolve();
  ...
  function portalAccessToken() {
    return window.ElainePortalAccess ? window.ElainePortalAccess.getToken() : '';
  }
  ...
  function storageKey() {
    return 'elaine-assignment-' + config.assignmentId + (testMode ? '-teacher-test' : '-learner');
  }
  ...
  async function finishPracticeNow() {
    ...
    var enteredName = window.prompt('Please type your name before submitting:', state.studentName || 'Elaine');
    if (enteredName === null) {
  ...
  function loadReviewAnswersWhenReady() {
    var ready = window.ElainePortalAccess && window.ElainePortalAccess.ready ? window.ElainePortalAccess.ready : Promise.resolve();
    ready.then(function () {
  ...
  function jsonp(action, parameters) {
    return portalReady.then(function () { return new Promise(function (resolve, reject) {
      ...
      var callbackName = '__elaineAssignment' + Date.now() + Math.random().toString(16).slice(2);
  ...
  function createId(key) {
    ...
    return 'elaine-' + key + '-' + Date.now() + '-' + Math.random().toString(16).slice(2);
  }
